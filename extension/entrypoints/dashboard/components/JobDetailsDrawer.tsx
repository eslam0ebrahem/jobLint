...
export function JobDetailsDrawer({ job, onClose, onMove, onDelete, onEvaluate, onUpdateNotes, onRecordOutcome, evaluationBusy, onError }: Props) {
  ...
  const { events, eventsLoading } = useJobEvents(job?.id, `${job?.updatedAt || ''}:${ledgerRevision}`);

  // Dossier, decision, and gate changes append to the transition ledger without
  // touching the job, so the drawer refreshes off their dedicated events.
  useEffect(() => {
    const currentJobId = job?.id;
    return subscribeGateway((event) => {
      if (event.type === 'jobs-changed') return;
      const affectsThisJob = 'jobId' in event ? event.jobId === currentJobId : currentJobId !== undefined;
      if (affectsThisJob) setLedgerRevision((value) => value + 1);
    });
  }, [job?.id]);
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [job?.id, job?.updatedAt, ledgerRevision]);
  ...
  const reposts = events.filter((event) => event.type === 'repost_detected').reverse();
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40 backdrop-blur-sm"><div className="fixed inset-0" onClick={onClose} aria-hidden="true" /><aside className="relative z-10 flex h-full w-full max-w-2xl flex-col border-l border-slate-200 bg-white shadow-2xl" aria-label={`Details for ${job.title}`}>
    ...
    <div className="flex-1 space-y-5 overflow-y-auto p-5 text-xs text-slate-800">
      ...
      {reposts.length > 0 && <section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4"><h3 className="text-xs font-bold uppercase tracking-wider text-amber-900">Posting changes ({reposts.length})</h3><p className="mt-1 text-[11px] leading-5 text-amber-900">This posting was re-clipped after its captured text changed. The local evaluation was recomputed from the new text.</p><ul className="mt-2 space-y-2">{reposts.map((event) => <li key={event.id} className="rounded-lg border border-amber-200 bg-white px-2.5 py-2 text-[11px]"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-semibold text-amber-900">{new Date(event.at).toLocaleString()}</span><span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-900">{isContentChange(event.metadata?.contentChanged) ? 'Description changed' : 'Details changed'}</span></div><p className="mt-1 break-words leading-5 text-slate-600">{event.metadata?.summary || 'No field detail was recorded.'}</p>{event.metadata?.fields && <p className="mt-0.5 text-[10px] text-slate-400">Changed: {event.metadata.fields}</p>}</li>)}</ul></section>}

      <section><h3 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">Activity timeline</h3>{eventsLoading ? <p className="text-xs text-slate-400">Loading activity…</p> : events.length ? <ol className="space-y-2 border-l-2 border-slate-200 pl-4">{[...events].reverse().map((event) => <li key={event.id} className="relative text-xs"><span className={`absolute -left-[21px] top-1 h-2 w-2 rounded-full ${event.type === 'repost_detected' ? 'bg-amber-500' : 'bg-indigo-400'}`} /><strong className="capitalize text-slate-700">{event.type.replace('_', ' ')}</strong><span className="ml-2 text-slate-400">{new Date(event.at).toLocaleString()}</span>{event.metadata?.reason && <span className="ml-2 text-slate-500">{event.metadata.reason}</span>}</li>)}</ol> : <p className="text-xs text-slate-400">No recorded events yet.</p>}</section>
...
/** Event metadata is typed as string-or-boolean; accept both shapes. */
function isContentChange(value: string | number | boolean | undefined): boolean {
